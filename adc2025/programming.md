...
- 2025-04-22 マインスイーパーをお試し実行できるページに[お試し盤面データの生成機能](#お試し盤面データの生成-) が追加されました。セルの選択リストをアップロードすると [スコアがカウント（採点）できる機能](#スコアのカウント（採点）-) が増えました。また、[セル選択ログのダウンロード](#セル選択ログのダウンロード-) や [スコア（一覧表）のダウンロード](#スコア（一覧表）のダウンロード-)ができるようになりました。地雷が置かれていない [安全セルの指定](#安全セルの指定-) も可能になりました。
...
- [「マインスイーパー」をお試し実行できるページ](#サンプル盤面) からお試し盤面の生成及び **盤面データ** のダウンロードができるようになりました。プログラム開発にご活用いただければと思います。**「ボード選択」** からボードを選ぶと画面に盤面が表示されます。**画面に表示されたボード** は一番下の **「盤面データ：download board data」** からダウンロード可能です。
...
- 盤面データにおける地雷の位置は疑似乱数を用いて自動発生しています。**ボード名** のフォーマットは `board_{Xサイズ}x{Yサイズ}_{地雷数}_{seed}` です。seed は疑似乱数へ与えるパラメータの初期値です。同じ初期値からは同じ盤面データが生成されます。
...
## スコアのカウント（採点） ![更新](images/update.PNG)
- [「マインスイーパー」をお試し実行できるページ](#サンプル盤面) ではスコアのカウントができるようになりました。実際の盤面を駆動してスコアをカウントしています。まだ実行スピードがかなり遅いですが、スピードアップできるようにしたいと考えております。
...
## スコア（一覧表）のダウンロード ![更新](images/update.PNG)
...
- プログラミング競技で使用する盤面データはまだご提供できておりません。
- [「お試し盤面データの生成」](#お試し盤面データの生成-) を元に複数の盤面データを選定して**盤面データをまとめたテキストファイル** の形で提供させていただく予定です。paiza 環境の仕様から **実行時間の最大値は2秒** ですが、その時間でどの程度のボリューム（サイズや盤面数）を扱うことが可能か検討したいと思います。
- <ins>**この制限時間内に解くことが可能なサイズや盤面数についてのデータをお持ちの方は情報をいただけますと助かります。** </ins> ボリューム選定に活かしたいと思いますので、良いデータが取れましたらお気軽に ADC 事務局あてご連絡をいただければと思います。
- 準備ができましたらこちらのサイトで開示いたしますので、更新情報をご覧いただければと思います。
<!-- - 盤面数が足りない場合はADC事務局までご連絡をいただければと思います。-->
