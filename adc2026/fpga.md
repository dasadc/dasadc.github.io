...
000191             <-- X個の盤面データ、これは4行目、Y 行目でこの盤面データは終了
                   <-- 空行を挟みます
4 4 2 board_name         <-- 次の盤面データ、この要領で複数の盤面データを連続して記述
0111
1291
1921
1110
...
## 結果の記録
- 競技用盤面データに対する実行結果を、**[上記](#競技用盤面データの提供)でダウンロードしたzipファイルに同梱の Excelシート（minesweeper_fpga_result.xlsx）** に記入ください。
...
- **以下2点を`adc.das.sldm@gmail.com` より、ADC 事務局宛にご提出ください**。
  ...
  - `実行結果を記録した Excelシート（minesweeper_fpga_result.xlsx）`
- 提出物に不足がある場合、または Excel シートの記載が不十分な場合は、**評価対象外** となります。**提出後に不備に気づいた場合は、提出期限内に限り再提出を受け付けます** 。

- メール本文には「参加者入力フォーム」に記載いただいたお名前もしくはチーム名を明記ください。
