...
## FPGA 競技の基本方針
- FPGA 競技は **2024年度新設された競技** です。従来は、配置配線問題を模した「ナンバーリンク」の解法や、数値関数を計算する演算器の設計など、アルゴリズムおよびアーキテクチャ設計を対象としたコンテストを実施してまいりました。2026年度は **「プログラミング競技」** と **「FPGA競技」** の二部門を開催いたします。コンテストの目的であるアルゴリズムデザインの追求という方向性に変更はございませんが、ソフトウェア分野に加え、 **ハードウェア分野においても研究成果の発信（論文投稿）が可能となるよう、対象領域を拡張しております**。
...
- 参考として、 **マインスイーパーVerilog実装テンプレート** を提供いたします。本テンプレートには、盤面の読み込み機能やランダムマスのオープン処理などの基本的な機能が含まれていますが、解法ロジックは実装されておりません。また、本テンプレートの使用は必須**ではありません**。
- なお、<ins>本テンプレートにおける盤面サイズや盤面入力仕様については、今年度の競技ルールに従って適宜修正のうえご利用ください</ins>。
