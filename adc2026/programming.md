...
- 2026-08-07  paize.IOに入力する競技用盤面データは10分割する必要がある旨を**競技用盤面データの提供について**に追記しました。
...
- (3) 難易度： **全セルにおける地雷セルの割合**から、次の6種類を設けました。
1. low： 5% - 10%
2. middle： 8% - 15%
3. high： 10% - 20%
4. dangerous： 13% - 25%
...
- 地雷セル数の下限は **3 個** となっています。low モードの場合、4x4=16 セルの 5% は 1 個未満ですが、下限制約から 3 個になります。
...
- paize.IOで1度に入力できる盤面数は約1000程度となります。競技用盤面データを複数のファイル(例：1000盤面×10ファイル)に分割いただき、プログラムを複数回に分けて実行してください。実行時間については、各回の実行時間の合計値(2秒以下)をご提出ください。
- 最大の盤面サイズは **19x19** です。
